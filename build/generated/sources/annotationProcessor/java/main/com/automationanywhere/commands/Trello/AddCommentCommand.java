...
public final class AddCommentCommand implements BotCommand {
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    if(parameters.containsKey("comment") && parameters.get("comment") != null && parameters.get("comment").get() != null) {
      convertedParameters.put("comment", parameters.get("comment").get());
      if(!(convertedParameters.get("comment") instanceof String)) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","comment", "String", parameters.get("comment").get().getClass().getSimpleName()));
      }
    }
    if(convertedParameters.get("comment") == null) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.notEmpty","comment"));
    }

    command.setSessions(sessionMap);
    command.setGlobalSessionContext(globalSessionContext);
    try {
      Optional<Value> result =  Optional.ofNullable(command.action((String)convertedParameters.get("sessionName"),(String)convertedParameters.get("board"),(String)convertedParameters.get("card"),(String)convertedParameters.get("comment")));
