...
public final class StartSessionCommand implements BotCommand {
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    if(parameters.containsKey("key") && parameters.get("key") != null && parameters.get("key").get() != null) {
      ...
      if(!(convertedParameters.get("key") instanceof SecureString)) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","key", "SecureString", parameters.get("key").get().getClass().getSimpleName()));
      }
    }
    ...
    try {
      command.action((String)convertedParameters.get("sessionName"),(SecureString)convertedParameters.get("key"),(SecureString)convertedParameters.get("token"));Optional<Value> result = Optional.empty();
