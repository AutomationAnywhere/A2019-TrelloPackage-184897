...
@CommandPkg(
        ...
        name = "Delete Team", label = "Delete Team",
        node_label = "Delete Team in session {{sessionName}}", description = "Deletes a team in Trello", icon = "trello.svg",
...
public class DeleteTeam {
    ...
    public void action(
            ...
            @Idx(index = "2", type = AttributeType.TEXT) @Pkg(label = "Team Name", description = "e.g. My Team Board. Note: You must have permissions to delete team") @NotEmpty String name
        ...
        String id = TrelloActions.GetTeamID(apiKey, token, name);
        //use the retrieved Board ID to make a call to delete that board
        String url = "https://api.trello.com/1/organizations/"+id+"?key="+apiKey+"&token="+token;
